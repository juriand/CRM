...
<body>
            ...
            <div id="page-inner">
                ...
                <div class="row">
			        ...
			        <div class="panel panel-default">
						...
						<div class="row">
						<div class="col-md-12">
						<form class="" id="clienform" action="clientInfoManage/client!updateCommuhistory" method="post">
							<div class="form-group col-md-5">
								<label>时间</label>
								<input id="date" name="commuHistory.commuHistory_date" maxlength="10" class="form-control" type="text"/>
							</div>
							
							<div class="form-group col-md-5">
								<label>地点</label>
								<input id="place" name="commuHistory.commuHistory_location" class="form-control" type="text"/>
							</div>
							...
							<div class="form-group col-md-5">
								<label>备注</label>
								<input id="memo" name="commuHistory.commuHistory_memo" class="form-control" type="text"/>	
							</div>

							<div class="form-group col-md-10">
								<label>详细信息</label>
								<textarea id="detail" name="commuHistory.commuHistory_detail" class="form-control" type="text"></textarea>
							</div>
						...
						<div class="form-group col-md-6">
							<button id="changeBtn" class="btn btn-primary" onclick="changeEditable();"><i class="glyphicon glyphicon-search"></i>修改</button>
							<button id="submitBtn" type="submit" form="clienform" class="btn btn-success"><i class="glyphicon glyphicon-home"></i>保存</button>
							<a href="clientInfoManage/client!deleteCommuhistory?commuhistoryid=<s:property value="#request.commuhistoryid"/>">
							    <button id="deleteBtn" class="btn btn-danger"><i class="glyphicon glyphicon-home"></i>删除</button>
							</a>
						</div>
						</div>
					</div>
    ...
    function initCommuhistory(){
    	var commuhistoryid = '<%= (String)request.getParameter("commuhistoryid") %>';
    	var url='clientInfoManage_ajax/client_ajax!showCommuhistoryInfo?commuhistoryid='+ commuhistoryid;
    	$.ajax({
    		type:'post',
    		url: url,
    		dataType:'json',
    		success:function(data){
    			...
    			if(isEmptyObject(data)){
    				$("#panelbody").html("<h3>获取交往记录失败</h3>");
    			}else{
    				console.log("commuHistory_date: "+data.date);
    				$("#date").attr({
    					"value":data.date,
	    				"disabled":true
	    			});
    				...
    				$("#place").attr({
    					"value":data.commuHistory.commuHistory_location,
	    				"disabled":true
	    			});
    				
    				$("#outline").attr({
    					"value":data.commuHistory.commuHistory_outline,
	    				"disabled":true
	    			});
    				
    				$("#memo").attr({
    					"value":data.commuHistory.commuHistory_memo,
	    				"disabled":true
	    			});
    				$("#detail").html(data.commuHistory.commuHistory_detail);
    				$("#detail").attr({
	    				"disabled":true
	    			});
    			}
